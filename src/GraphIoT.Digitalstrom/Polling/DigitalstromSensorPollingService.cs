...
namespace PhilipDaubmeier.GraphIoT.Digitalstrom.Polling
{
    public class DigitalstromSensorPollingService : IDigitalstromPollingService
    {
        ...
        public async Task PollValues()
        {
            ...
            {
                ex = ex.Demystify();
                _logger.LogInformation($"{DateTime.Now} Exception occurred in Digitalstrom sensor background worker: {ex}");
            }
        }
