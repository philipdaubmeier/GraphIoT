    ...
    public class WeConnectViewModel : GraphCollectionViewModelMultiResolutionBase<WeConnectData>
    {
        ...
        public override GraphViewModel Graph(int index)
        {
            return index switch
            {
                ...
                2 => DeferredLoadGraph<TimeSeries<int>, int>(index, _localizer["Consumed kWh"], null, _localizer["#.# kWh"]),
                3 => DeferredLoadGraph<TimeSeries<int>, int>(index, _localizer["Average consumption kWh/100 km"], null, _localizer["#.# kWh/100 km"]),
                4 => DeferredLoadGraph<TimeSeries<int>, int>(index, _localizer["Average speed km/h"], null, _localizer["#.# km/h"]),
                5 => DeferredLoadGraph<TimeSeries<int>, int>(index, _localizer["Charging state"], null, _localizer["# (charging/not charging)"]),
                6 => DeferredLoadGraph<TimeSeries<int>, int>(index, _localizer["Climate temperature"], null, _localizer["#.# °C"]),
                7 => DeferredLoadGraph<TimeSeries<int>, int>(index, _localizer["Climate state"], null, _localizer["# (on/off)"]),
                8 => DeferredLoadGraph<TimeSeries<int>, int>(index, _localizer["Window melt state"], null, _localizer["# (on/off)"]),
                9 => DeferredLoadGraph<TimeSeries<int>, int>(index, _localizer["Remote heating state"], null, _localizer["# (on/off)"]),
