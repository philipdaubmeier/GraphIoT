...
namespace PhilipDaubmeier.GraphIoT.Core.Database
{
    public class DatabaseBackupService<TDbContext> where TDbContext : DbContext
    {
        ...
        {
            return DatabaseBackupService<TDbContext>.GetTables(db, tableFilter).Select(table => new BackupTable()
            {
                Table = table.property.Name,
                Columns = DatabaseBackupService<TDbContext>.GetColumnNames(table.value),
                Rows = DatabaseBackupService<TDbContext>.GetColumns(table.value, start, end)
            });
        }
        ...
        {
            var backupData = await JsonSerializer.DeserializeAsync<BackupRoot>(backupToLoad, cancellationToken: cancellationToken) ?? throw new IOException("Backup could not be parsed - wrong format");

            var tableFilters = backupData?.Tables?.Select(table => table.Table.Trim())?.ToList() ?? new List<string>();

            foreach (var (property, value) in DatabaseBackupService<TDbContext>.GetTables(db, tableFilters))
            {
                var tableData = backupData?.Tables.Where(tableToRestore => tableToRestore.Table == property.Name).FirstOrDefault();
                if (tableData == null)
                    continue;

                var columnNames = DatabaseBackupService<TDbContext>.GetColumnNames(value).ToList();
                ...
                var columnProps = DatabaseBackupService<TDbContext>.GetColumnProperties(value).ToList();
        ...
        private void RestoreBackupRow(IQueryable<object> tableDbSet, List<object> rowData, PropertyInfo tableProperty, List<PropertyInfo> columnProperties)
        {
            var entityType = tableProperty.PropertyType.GenericTypeArguments.First();
            var newEntity = Activator.CreateInstance(entityType) ?? throw new Exception($"Could not instantiate {entityType}");
        ...
        private static IEnumerable<(PropertyInfo property, IQueryable<object> value)> GetTables(DbContext dbContext, ICollection<string> tableFilter)
        {
        ...
        private static IEnumerable<PropertyInfo> GetColumnProperties(IQueryable<object> table)
        {
        ...
        private static List<string> GetColumnNames(IQueryable<object> table)
        {
            return DatabaseBackupService<TDbContext>.GetColumnProperties(table).Select(prop => prop.Name).ToList();
        }

        private static List<List<object>> GetColumns(IQueryable<object> table, DateTime start, DateTime end)
        {
            var columnProperties = DatabaseBackupService<TDbContext>.GetColumnProperties(table);
            ...
            return rows.ToList().Select(row => columnProperties.Select(prop => prop.GetGetMethod())
                                                               ...
                                                               .Select(getter => getter!.Invoke(row, null) ?? new()).ToList()).ToList();
        }
    }
}