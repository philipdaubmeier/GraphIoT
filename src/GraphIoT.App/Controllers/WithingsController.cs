...
namespace PhilipDaubmeier.GraphIoT.App.Controllers
{
    ...
    public class WithingsController : Controller
    {
        ...
        private IOptions<TelegramConfig> _telegramConfig;

        public WithingsController(WithingsClient withingsClient, IOptions<TelegramConfig> telegramConfig)
        {
        ...
        public async Task<IActionResult> PostCallback(string code, string state)
        {
            ...
            async Task sendTelegramMessage(string message) => await (new HttpClient()).GetStringAsync($"https://api.telegram.org/{_telegramConfig.Value.BotId}/sendMessage?chat_id={_telegramConfig.Value.ChatId}&text=" + WebUtility.UrlEncode(message));
