...
namespace PhilipDaubmeier.SmarthomeApi.Services
{
    public class ViessmannTimedHostedService : IHostedService, IDisposable
    {
        ...
        public ViessmannTimedHostedService(IServiceProvider services, ILogger<ViessmannTimedHostedService> logger)
        {
            _services = services;
            _logger = logger;
        }
        ...
        private async void PollAll(object state)
        {
            ...
            {
                await PollValuesScoped<ViessmannSolarPollingService>(scope);
                await PollValuesScoped<ViessmannHeatingPollingService>(scope);
            }
        }
